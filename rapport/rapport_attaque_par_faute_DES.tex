	...
	\newpage
	 \tableofcontents
		...
		\chapter{Fautes sur les tours précédents}
			...
			Pour retrouver la clef $K_{15}$ on va écrire une équation la contenant a l'aide de $R_{16} et R_{16}'$.
			On a donc:\\ $R_{16} \oplus R_{16}'=R_{15} \oplus R_{15}'=L_{14}\oplus F(R_{14},K_{15}) \oplus L_{14}\oplus F(R_{14}',K_{15})$.\\
			Grace au XOR les deux L14 s'envont, on obtient donc :$F(R_{14},K_{15}) \oplus F(R_{14}',K_{15})$\\
			On développe la Fonction F et on enlève la permutaion P car elle est linaire donc j'applique $P^{-1}$ a l'équation, on a donc :$S(E(R_{14})\oplus K_{15})\oplus S(E(R_{14}')\oplus K_{15})$.\\
			
			Maintenant pour ce faciliter la tâche nous allons essayer de déterminer les bit fautés.
			si on fait $R_{16} \oplus R_{16}'$ théoriquement cela nous renvoie les bits différent, donc ceux fautées (on notera $\mathcal{E}$).\\
			De plus on sait que $R_{16}=R_{15} et R_{16}' =R_{15}'$ donc $R_{15} \oplus R_{15}' = \mathcal{E}$.
			Je récris cette égalité sous cette forme $ R_{15}' = R_{15} \oplus \mathcal{E}$, puis je développe $R_{15}$.
			On a donc a la fin $R_{15}' = L_{14}\oplus F(R_{14},K_{15}) \oplus \mathcal{E}$. De cette égalité je peux déterminer exactement les sbox touchées par le bit fauté.\\
			Maintenant que je connais ou est la faute, je vais pour utiliser l'équation contenant $K_{15}$. Je vais brute force $2^{6}$ valeurs sur les sbox touché par le bit fauté et retenir tous les $\alpha$ égaux a l'output de $S(E(R_{14}')\oplus K_{15})$( si on se fit a la description, de wikipedia , il y a par sorti 4 input possible).\\
			Ne connaissant ni $R_{14} et R_{14}'$, je vais simuler l'expansion de $R_{14}'$ en fesant une force brute sur $2^{6}$ valeurs(les 6 bits d'expansion touchant la Sbox avec le bit fauté.)que je vais xor avec chaque $\alpha$ de chaque Sbox touchée par un bit fauté.
			Cela nous donnera N candidats (dans notre cas $2^{2}*2{6}$ candidats) de 6 bit pour chaque Sbox touchée par un bit fauté pour retrouvé la clef $K_{15}$.\\
			Une fois les étapes précédentes faites je vais re-itérer ces étapes autant de fois pour avoir des candidats pour chaque Sbox (Dans le pire cas $2^{3}$ fois).
		...
		\chapter{Contre-mesures}
			...
			\section{Protection structurelle}
			Une autre solution est de créé une carte qui dès qu'elle est attaqué, ce bloc et ne revoie strictement rien a l'assaillant.
		...
		-Des.pdf
		-Cours de Licence 3 et M1 de cryptographie
